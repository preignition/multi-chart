...
class TemplateInstance {
  ...
  _clone() {
    ...
    const stack = [];
    const {parts} = this.template; // Edge needs all 4 parameters present; IE11 needs 3rd parameter to be null
...
const isIterable = value => Array.isArray(value) || // tslint:disable-next-line:no-any
...
const render$1 = (result, container, options) => {
  ...
  render(result, renderContainer, {templateFactory: shadyTemplateFactory(scopeName), ...options}); // When performing first scope render,
...
const standardEventOptions = (options, element) => ({ ...element, finisher(clazz) {
...
const unsafeCSS = value => new CSSResult(String(value), constructionToken);
