...
import { c as color } from './common/index-a99c6aaf.js';
...
Dispatch.prototype = dispatch.prototype = {
  ...
  on: function on(typename, callback) {
    const {_} = this;
        ...
        let t;
        let i = -1;
        const n = T.length; // If no callback was specified, return the callback of the given type and name.
...
let taskHead;
    ...
    const setFrame = typeof window === "object" && window.requestAnimationFrame ? window.requestAnimationFrame.bind(window) : function (f) {
  setTimeout(f, 17);
};
function now() {
...
function create(node, id, self) {
  const schedules = node.__transition;
      let tween; // Initialize the self timer when the transition is created.
  ...
  function start(elapsed) {
    let i; let j; let n; let o; // If the state is not SCHEDULED, then we previously errored on start.
...
function tweenRemove(id, name) {
  let tween0; let tween1;
  return function () {
    const schedule = set$1(this, id);
        const {tween} = schedule; // If this node shared tween with the previous node,
...
function attrFunction(name, interpolate, value) {
  let string00; let string10; let interpolate0;
  return function () {
    let string0;
...
function transition_attr (name, value) {
  const fullname = namespace(name);
      const i = fullname === "transform" ? interpolateTransformSvg : interpolate;
...
function onFunction(id, name, listener) {
  let on0;
      let on1;
      const sit = start(name) ? init : set$1;
  return function () {
    const schedule = sit(this, id);
        const {on} = schedule; // If this node shared a dispatch with the previous node,
...
function styleMaybeRemove(id, name) {
  ...
  return function () {
    const schedule = set$1(this, id);
        const {on} = schedule;
        const listener = schedule.value[key] == null ? remove || (remove = styleRemove(name)) : undefined; // If this node shared a dispatch with the previous node,
    // just assign the updated shared dispatch and we’re done!
    // Otherwise, copy-on-write.
